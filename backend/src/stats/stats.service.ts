...
export class StatsService {
	...
	async updateStats(match: Match, User_id: number, indexPlayer: number, leaverId: number) {
		const stats: StatsDetail = await this.getUserStats(User_id);
		...
		if (match[`scorePlayer${indexPlayer}`] != match[`scorePlayer${opId}`] || leaverId != -1) {
			if ((match[`scorePlayer${indexPlayer}`] > match[`scorePlayer${opId}`] && User_id !== leaverId) || match['player' + opId].id === leaverId)
				stats.wins += 1;
			else
				stats.looses += 1;
		}
