...
export class AuthService {
	...
	async getUserObject(authDto: AuthLoginDto, res: Response) {
		...
		if (errors.length)
			throw new NotAcceptableException(await this.get_ErrorMsg(errors))
		const tokens: string = await this.loginWithCredentials(authDto, res);
		const user: User = await this.usersService.getByEmail(authDto["email"]);
		return { tokens: tokens, user: user };
	}

	async loginWithCredentials(authLoginDto: any, @Res() res: Response): Promise<string> {
	...
	async loginWithFortyTwo(@Req() req, @Res({ passthrough: true }) res, data: AuthLogin42Dto) {
		const user = await this.usersService.getByEmail(data.email);
		if (!user) {
			if (await this.usersService.fetchByName(data.name))
