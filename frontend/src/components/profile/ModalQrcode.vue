<template>
	<div class="qrcode_modal_overlay" @click="closeModal">
		<div class="qrcode_modal" @click.stop>
			<SlidingTitle text="2FA" class="qrcode_title"></SlidingTitle>
			<div class="instructions">
				Scan the code with the Google Authenticator app on mobile.
			</div>
			<img alt="qrcode" v-if="isDataLoaded" :src="getUrl()" class="qrcode">
			<div class="instructions">
				Enter the code found on the application.
			</div>
	...
	methods: {
		...
		async sendCode() {
			...
			if (json["status"] === "Failure") {
				...
				let message: string = !this.isActivated ? "Double Authentication Enabled!" : "Double Authentication Disabled!"
				notification.notify({
					title: message,
					type: 'success',
					group: 'notif-center'
