	...
	<Transition name="slide-fade" mode="out-in">
		<div v-if="show" class="modal_overlay" @click="$emit('close')">
			<div class="modal_friend" @click.stop>
				<p>Attention, <b> {{ username }}</b> Will be  :</p>
				<div class="button_box">
					<BlueButton text="Block" v-on:click="block_User(); delete_Friend(); $emit('close')"></BlueButton>
					<BlueButton text="Remove" v-on:click="delete_Friend(); $emit('close')"></BlueButton>
				</div>
			</div>
		</div>
	</Transition>
