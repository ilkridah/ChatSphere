...
<template>
	...
	<div v-if="show" class="modal_overlay_channel" @click="$emit('close')">
		<div class="modal_channel" @click.stop>
			<SlidingTitle text="Join a Channel"/>
			<div class="field_channel">
				<input v-model="channel_name" class="entry_channel" type="text" maxlength="15" placeholder="Channel Name"/>
				<input class="entry_channel" type="password" placeholder="Password (Optional)" v-model="channel_password"/>
				<BlueButton text="Confirm" icon="" @click="addChannel()" />
			</div>
		</div>
	</div>
	</Transition>
