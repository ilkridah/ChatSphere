	...
	methods: {
		async addUser() {
			...
			if (response.status == 406) {
				this.addFriendNotif("Please insert a name", "error");
				return ;
			}
...
<template>
	...
	<div v-if="show" class="modal_overlay_friend" @click="$emit('close')">
		<div class="modal_friend" @click.stop>
				<SlidingTitle text="Add a Friend"/>
				<div class="input_friend">
					<input class="entry_friend" type="text" placeholder="User Name" v-model="username">
					<BlueButton text="Add" icon="" @click="addUser" />
				</div>
		</div>
	</div>
	</Transition>
