...
export default defineComponent({
	...
	methods: {
		async addPassword(password: string) {
			...
			const response = await fetch('http://' + import.meta.env.VITE_HOST + ':3000/chat/' + this.$props.channel_id + "/" + this.$props.sender.id + '/change_Password', {
				credentials: 'include',
				method: 'POST',
				headers: {
...
<template>
	<Transition name="slide-fade" mode="out-in">
		<div v-if="show" class="modal_overlay" @click="$emit('close')">
			<ChannelOptionModal @click.stop title="Change Password"
