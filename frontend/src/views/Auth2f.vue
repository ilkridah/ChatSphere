	...
	<div class="auth">
		<div class="auth-card">
			...
			<div class="auth-container">
				...
				<div class="info-box">
					<BlueButton v-if="!generated" text="Enable" icon="fa-solid fa-check" class="card-button" @click="redirect({ path: '/auth/2fa/generate'})"/>

					<BlueButton v-else-if="activated === false" text="Enable" icon="fa-solid fa-check" class="card-button"
						@click="redirect({ path: '/auth/2fa/verif', query: { plan: 'on' }})"/>

					<BlueButton v-else-if="activated === true" text="Disable" icon="fa-solid fa-xmark" class="card-button"
...
export default{
	...
	methods: {
		getTwoFactorAuthStatus: async (vm: any) => {
	...
	mounted() {
		this.getUser(this);
		this.getTwoFactorAuthStatus(this);
	},
